...
def make_dil(image_dim):
    ...
    for i in range(n_layers):
        OUT = Conv2D(nK[i], kernel_size=[kDim[i], kDim[i]], dilation_rate=(n_dil[i], n_dil[i]), activation='relu',
                     padding='same')(OUT)
...
def make_model(image_dim, nlabels, nK, n_dil, kernel_size, drop_out, model_type='model_0_0', activation_hidden="relu",
               activation_output="sigmoid", verbose=0):
    if model_type == 'unet':
...
def compile_and_run(model, model_name, history_fn, X_train, Y_train, X_validate, Y_validate, nb_epoch, nlabels,
                    metric="categorical_accuracy", loss='categorical_crossentropy', lr=0.005, verbose=0):
    # set compiler
    ...
    history = model.fit([X_train], Y_train, validation_data=([X_validate], Y_validate), epochs=nb_epoch,
